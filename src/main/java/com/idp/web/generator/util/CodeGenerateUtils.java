...
public class CodeGenerateUtils extends CodePathUtils {
	...
	public void generateFile() throws Exception {
		...
		if("other".equals(this.config.getPageModel())){
	...
	public void invoke(String templateFileName,String type) throws TemplateException, IOException {
		...
		if("project".equals(this.config.getPackageStyle())){
	...
	private Map<String, Object> initData(BuildCodeConfig buildCodeConfig){
		...
		data.put("createDate", FormatDate(new Date(), "yyyy-MM-dd"));
		data.put("author", buildCodeConfig.getAuthor());
		
		String webPackage = buildCodeConfig.getWebBasePackage().replace("webpage/", "");
		
		if(webPackage.indexOf("/") == 0){
			webPackage = webPackage.substring(1);
		}
		data.put("webPackage", webPackage);
